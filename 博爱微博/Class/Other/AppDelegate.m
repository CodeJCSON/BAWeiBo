...
#import "BAOAuthViewController.h"
#import "BARootTool.h"
...
/*
 LaunchScreen:代表之前的启动图片
 
 程序中碰到模拟器尺寸不对，马上去找启动图片，默认模拟器的尺寸由启动图片决定
 
 */

#define BAVersonKey @"Verson"
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    // 选择根控制器
    // 判断下有没有授权
    if ([BAAccountTool account]) { // 已经授权
        
        // 选择根控制器
        [BARootTool chooseRootViewController:self.window];
        
    }else{ // 进行授权
        BAOAuthViewController *oauthVc = [[BAOAuthViewController alloc] init];
...
- (void)applicationWillResignActive:(UIApplication *)application {
    ...
    NSURL *url = [[NSBundle mainBundle] URLForResource:@"爸比我要喝奶奶" withExtension:nil];
